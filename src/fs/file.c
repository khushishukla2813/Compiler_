...
#include "status.h"
#include "kernel.h"
struct filesystem* filesystems[COMPILER_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[COMPILER_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem()
{
    int i = 0;
    for (i = 0; i < COMPILER_MAX_FILESYSTEMS; i++)
    {
        if (filesystems[i] == 0)
        {
...
{
    int res = -ENOMEM;
    for (int i = 0; i < COMPILER_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
    if (fd <= 0 || fd >= COMPILER_MAX_FILE_DESCRIPTORS)
    {
        return 0;
    }
...
struct filesystem* fs_resolve(struct disk* disk)
{
    ...
    for (int i = 0; i < COMPILER_MAX_FILESYSTEMS; i++)
    {
