...
static struct process* processes[COMPILER_MAX_PROCESSES] = {};
...
{
    if (process_id < 0 || process_id >= COMPILER_MAX_PROCESSES)
    {
        return NULL;
    }
...
{
    int res = -ENOMEM;
    for (int i = 0; i < COMPILER_MAX_PROGRAM_ALLOCATIONS; i++)
    {
...
{
    for (int i = 0; i < COMPILER_MAX_PROCESSES; i++)
    {
        if (processes[i])
        {
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*) COMPILER_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
int process_map_memory(struct process* process)
{
    ...
    paging_map_to(process->task->page_directory, (void*)COMPILER_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack+COMPILER_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
out:
    return res;
}
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    _process->stack = kzalloc(COMPILER_USER_PROGRAM_STACK_SIZE);
    if (!_process->stack)
    {
        res = -ENOMEM;
        goto out;
