...
{
    return header->e_type == ET_EXEC && header->e_entry >= COMPILER_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header) && elf_is_executable(header)) ? COMPILER_ALL_OK : -EINFORMAT;
}
