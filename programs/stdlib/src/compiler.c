...
struct command_argument* compiler_parse_command(const char* command, int max)
{
    ...
    root_command = compiler_malloc(sizeof(struct command_argument));
    if (!root_command)
    {
        goto out;
    }
    ...
    while(token != 0)
    {
        struct command_argument* new_command = compiler_malloc(sizeof(struct command_argument));
        if (!new_command)
        {
            break;
        }
...
int compiler_system_run(const char* command)
{
    ...
    struct command_argument* root_command_argument = compiler_parse_command(buf, sizeof(buf));
    if (!root_command_argument)
    {
        return -1;
    }

    return compiler_system(root_command_argument);
}
